...
def max_min_ave(t_li):
    t = np.array(t_li)
    summary_li = [np.max(t), np.min(t), np.mean(t), np.std(t)]
    return summary_li


def time_word(tw_li_li, num):
    time_fn = num + '_time'
    word_fn = num + '_word'
    summary_time_fn = time_fn + '_summary'
    summary_word_fn = word_fn + '_summary'
    time_li = []
    word_li = []
    fn_t = open(time_fn + '.txt', 'a')
    fn_w = open(word_fn + '.txt', 'a')
