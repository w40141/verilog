...
def max_min_age(t_li):
    t_max = max(t_li)
    t_min = min(t_li)
    t_age = sum(t_li) / len(t_li)
    t_li = [t_max, t_min, t_age]
    return t_li


def time_word(tw_li_li):
    time_li = []
    word_li = []
    for tw_li in tw_li_li:
        for tw in tw_li:
            time_li.append(tw[0])
            word_li.append(tw[1])
    t_li = max_min_age(time_li)
    w_li = max_min_age(word_li)
    return t_li, w_li
...
def experience(c):
    ...
    for i in range(num):
        ...
        for moji in moji_li:
            ans = 0
            for test_message in range(4, 17):
                run_li_li = []
        ...
        tmp = [elapsed_time, i * 89 + len(message_li)]
        tw_li.append(tmp)
    return tw_li


def main():
    message_li = tmp_make_message_li(16, 'a')
    print(len(message_li[0]))
    m = message_input(message_li[0])
    print(len(m))
    print(m)
    # c_num = 2
    # pool = mp.Pool(c_num)
    ...
    # print(time_li_li)
    # t_li, w_li = time_word(time_li_li)
    # print(t_li)
    # print(w_li)
